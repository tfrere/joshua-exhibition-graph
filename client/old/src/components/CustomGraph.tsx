    ...
    coordinates: {
      ...
      [coordinateType]: {
        x:
          rangeX === 0
            ? 0
            : ((post.coordinates[coordinateType].x - minX) / rangeX - 0.5) *
              scale,
        y:
          rangeY === 0
            ? 0
            : ((post.coordinates[coordinateType].y - minY) / rangeY - 0.5) *
              scale,
        z:
          rangeZ === 0
            ? 0
            : ((post.coordinates[coordinateType].z - minZ) / rangeZ - 0.5) *
              scale,
      },
    },
  }));
...
const analyzeData = (posts: Post[], coordinateType: CoordinateType) => {
  ...
  if (invalidPosts.length > 0) {
    console.warn(
      `⚠️ ${invalidPosts.length} posts ont des coordonnées invalides!`
    );
  ...
  const avgZ =
    coordinates.reduce((sum, coord) => sum + coord.z, 0) / coordinates.length;
...
export default forwardRef(function CustomGraph(
  ...
  const { activePost, findClosestNode } = useClosestNode(
    posts,
    camera,
    coordinateType
  );
  ...
  useEffect(() => {
    ...
    const initialNormalizedData = normalizeCoordinates(
      sortedPosts,
      initialCoordinateType.current
    );
    ...
    debugLog(
      `Analyse des données initiales ${analyzeData(
        initialPosts,
        initialCoordinateType.current
      )}`
    );
  ...
  const handleCoordinateTypeChange = useCallback(
    (newType: CoordinateType) => {
