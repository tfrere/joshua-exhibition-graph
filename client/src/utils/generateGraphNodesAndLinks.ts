...
interface GraphDataOptions {
  includePosts?: boolean; // Option pour inclure ou non les posts
}
...
export function generateGraphData(
  ...
  // Valeur par défaut pour l'option d'inclusion des posts
  const includePosts =
  ...
  // Ne traiter les posts que si l'option est activée
  if (includePosts) {
    characters.forEach((character) => {
  ...
  // Combiner tous les liens - uniquement les liens entre personnages et sources, et entre posts et personnages si l'option est activée
  const links = [
    ...characterLinks,
    ...sourceLinks,
  ...
  console.log("Résumé de la génération:", {
    ...
    liensPosts: includePosts ? postLinks.length : 0,
  });
