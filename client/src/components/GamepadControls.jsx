...
/**
 * Contrôle de la caméra par manette de jeu (gamepad)
 * @param {Object} props - Les propriétés du composant
 * @param {Object} props.config - Configuration du contrôleur
 * @param {number} props.config.maxSpeed - Vitesse maximale
 * @param {number} props.config.acceleration - Accélération
 * @param {number} props.config.deceleration - Décélération
 * @param {number} props.config.rotationSpeed - Vitesse de rotation
 * @param {number} props.config.deadzone - Zone morte des joysticks
 */
export function GamepadControls({ config }) {
  ...
  useFrame((state, delta) => {
    ...
    const thrust =
      Math.abs(gamepad.axes[1]) > config.deadzone ? -gamepad.axes[1] : 0;
